...
### Added
- **Indoor Temperature / Indoor Humidity panels** (Weather Mode) — two new rotating panels showing the clock's internal SHT30 sensor reading, alongside the existing outdoor Temperature/Humidity/Sunrise & Sunset/Wind/Daily Hi-Lo panels. Off by default; enable them under Services → Weather → Display Panels (a unit without the sensor never rotates into them). Laid out like their outdoor counterparts, and every Weather Mode temperature/humidity panel now carries a small "In"/"Out" label so indoor and outdoor readings are never mistaken for each other.
- **Pause Rotation** control (Dashboard → Quick Actions) — freezes the current mode without touching the Mode Rotation setting itself, so the configured interval, weights, and mode list are all preserved; a timed pause auto-resumes on its own, and resuming (manually or automatically) always picks the schedule back up on a fresh dwell instead of possibly jumping immediately. Doesn't persist across a reboot (always starts unpaused). The durations it offers are configurable under Display → Mode Rotation → "Pause Rotation timers": an "until turned back on" option and a comma-separated list of hour values (e.g. `1,2,8`). When only one option is configured the button is a plain pause/resume toggle with no menu.

### Fixed
- Web UI: on narrow phone screens (iOS Safari, ~390px wide), the Display → Advanced Panel Profile and Window Offset tables' last column (VCOM / Row) could run off the right edge of the screen — the per-tube control blocks now use a much smaller side indent on small screens, matching the space actually needed. The Color Inversion checkboxes' text also visibly overlapped on real iOS Safari even after that fix (a per-item width that fit fine in desktop testing didn't hold up against the device's actual font/checkbox rendering) — they now use a fixed 2-column grid on small screens instead of trying to fit 3 across a single flexible row. iOS Safari also rendered a large gap between each checkbox and its "Tube N" label (the checkbox's native box was sized well beyond its visible square) — checkboxes site-wide now get an explicit, fixed size instead of leaving it to each browser's own native control sizing. (#88)
- A sustained Home Assistant MQTT reconnect failure (e.g. the broker's hostname no longer resolving) could permanently pin one of the web server's 12 available connection slots per Firmware/Filesystem Update, LittleFS hotpatch, or Verify & Repair Stock Files request made while MQTT was stuck reconnecting — each of those waited indefinitely for MQTT to fully stop before proceeding, so a wedged MQTT client meant that wait, and the browser's connection, never returned. The wait is now capped at 3 seconds; past that it proceeds without further waiting instead of hanging. This underlying wait has existed for longer than this release, but a change in this release also made the Partial/Full Factory Reset paths depend on it for the first time.
- Web UI: the dashboard's background status poll (every 5s) had no timeout and no guard against overlapping requests — if the device was ever slow to answer for any reason, every 5s added another pending connection on top instead of waiting for the previous one, so a single slow response could snowball into several. It now skips a tick if the last poll hasn't returned yet, and gives up after 8s instead of waiting indefinitely.
- Web UI: on iOS 15 and older, the page loaded but never actually connected — the status poll above used `AbortSignal.timeout()`, unsupported before Safari 16, which throws immediately and silently aborted the whole first-connect sequence before it ever reached the dashboard. All uses of it across the page (status poll, firmware/filesystem update, stock-file repair, first-time-setup redirect) now fall back to an equivalent manually-timed abort on older Safari instead.
- Web UI: System → Web UI Update ("Upload & Apply") could pop a spurious "API error" toast partway through — unlike the firmware/filesystem update flows, the Web UI hotpatch never paused the dashboard's background status poll, so a `/api/status` request that stalled behind the ~minute-long LittleFS write would hit the poll's new 8-second timeout and surface as an error even though the update itself was fine. The hotpatch now suppresses the poll for its duration like the other update flows, and `/api/status` no longer opens `version.txt` on every call (it's cached alongside the filesystem-usage figures, re-read only when a hotpatch/pull/OTA changes it).
- Hardened the "stop Home Assistant MQTT before a flash write" step used by every OTA / Web UI Update / hotpatch / stock-file repair. The bounded stop could still fall through to a direct, unbounded `esp_mqtt_client_stop()` if its worker task couldn't be created (most likely right after boot, when internal RAM is most fragmented and the MQTT client is most likely wedged mid-first-connect — exactly when that call blocks). It now retries the worker briefly and otherwise skips stopping MQTT rather than blocking the update; the wait is also shorter (1.5 s) and `ha_mqtt_resume()` waits for any overrunning background stop so the two can't race. Separately, the online updater's post-reboot Web UI pull could hang with the wait screen never appearing on the tubes: a `/api/status` filesystem-usage poll landing in the narrow window just before the pull marks itself active would park the display via the reference-counted path, then unpark it when that poll finished — resuming the display task while the pull was still drawing the wait screen, so both hit the SPI bus at once ("previous polling transaction not terminated") and the pull wedged on the corrupted transaction. The wait-screen park is now a distinct one-way hold that the reference-counted pause/unpause can't release, and `/api/status` skips its walk outright while any update is active.
- Saving settings re-applied the NTP servers, timezone, audio-enabled flag, and SHT30 temperature offset every time, even when none of those fields changed — restarting the SNTP engine and logging "NTP servers updated" / "Timezone updated" / "audio_set_enabled: change saved" / "Temperature offset set to …" on every unrelated save. Each is now applied only when its own value actually changed, matching how the display appliers (VCOM, gamma, offsets, …) already worked.
- Internal-RAM fragmentation grew slowly over long uptime with a dashboard tab left open, since `/api/status` (polled every 5s) rebuilds its whole JSON response from scratch each time, and every one of those small tree-node allocations previously landed in the same limited, fragmentation-prone internal-RAM region as WiFi/crypto's DMA-sensitive buffers, for no reason — nothing about a JSON tree needs DMA. All JSON building/parsing firmware-wide (this endpoint, weather data, etc.) now allocates from PSRAM instead.
