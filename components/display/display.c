...
static void render_weatherlive(const nextube_config_t *cfg, const struct tm *t, bool demo)
{
    ...
    /* Night color set (issue #73): crossfade day→night colors by the scene's
     * twilight-aware darkness (wl_scene_t.night, 0..255, driven by the real
     * geocoded sunrise/sunset — the same value that fades the stars in).
     * WL-sky background only — static theme-image / CustomColor backgrounds
     * keep the day set, which also keeps the colon-frame cache path (only
     ...
     * below collapse to exactly the day values. */
    {
        ...
        s_wl_glyph_r = (uint8_t)out[0]; s_wl_glyph_g = (uint8_t)out[1]; s_wl_glyph_b = (uint8_t)out[2];
        for (int i = 0; i < 3; i++) { day[i] = cfg->custom_font_color[i];
                                      nite[i] = cfg->custom_font_color_night[i]; }
        wl_lerp3(day, nite, nt, out);
        s_wl_font_r = (uint8_t)out[0]; s_wl_font_g = (uint8_t)out[1]; s_wl_font_b = (uint8_t)out[2];
        for (int i = 0; i < 3; i++) { day[i] = cfg->custom_shadow_color[i];
